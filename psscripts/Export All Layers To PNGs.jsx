...
// Type definitions
//
var CONST_OPTION_SHOW_DIALOG = false;
var CONST_OPTION_DEFAULT_OPTION_SETTER_FUNC = setDefaultOptsPNG24;
...
function main() {
    ...
    // setting
	if (CONST_OPTION_SHOW_DIALOG && !showDialog()) {
		return "cancel";
	}else{
		(CONST_OPTION_DEFAULT_OPTION_SETTER_FUNC || setDefaultOptsPNG24)();
	}
...
function exportLayers(exportLayerTarget, progressBarWindow) {
    ...
    var count = prefs.bgLayer
        ? layersToExport.length - 1
    ...
    if ((layerCount == 1) && layers[0].layer.isBackgroundLayer) {
        ...
        if (prefs.trim == TrimPrefType.COMBINED) {
            var UPDATE_NUM = 20;
            if (progressBarWindow) {
                var stepCount = (exportLayerTarget == ExportLayerTarget.ALL_LAYERS)
                    ? count / UPDATE_NUM + 1
                    : 1;
...
function getUniqueFileName(fileName, layer) {
    ...
    var outputPrefix = prefs.groupsAsFolders
        ? ""
        : prefs.outputPrefix;
...
function showDialog() {
    ...
    var formatFuncs = [getFormatOptsPNG24, getFormatOptsPNG8, getFormatOptsJPEG, getFormatOptsTarga, getFormatOptsBMP];
    var formatOpts = [
        {
            opt: null,
            controlRoot: null
...
function applySettings(dlg, formatOpts) {
    ...
    with (dlg.funcArea.content) {
        ...
        grpNaming.drdNaming.selection = (drdNamingIdx >= 0)
            ? drdNamingIdx
            : 0;
        ...
        grpLetterCase.drdLetterCase.selection = (drdLetterCaseIdx >= 0)
            ? drdLetterCaseIdx
            : 0;
        ...
        grpTrim.drdTrim.selection = (drdTrimIdx >= 0)
            ? drdTrimIdx
            : 0;
...
function getFormatOptsJPEG() {
    return {
        ...
        dialogParams: function(parent) {
            ...
            parent.matte = row.add("dropdownlist", undefined, [
                "White",
                "Black",
                "Gray",
                "-",
...
function setDefaultOptsPNG24(){
	...
	prefs.naming = FileNameType.AS_LAYERS_NO_EXT;
    prefs.namingLetterCase = LetterCase.LOWERCASE;
	prefs.outputPrefix = "";
    prefs.replaceSpaces = true;
    prefs.groupsAsFolders = true;
...
function getFormatOptsPNG8() {
    return {
        ...
        dialogParams: function(parent) {
            ...
            parent.dither = row.add("dropdownlist", undefined, ["None", "Diffusion", "Pattern", "Noise"]);
            ...
            matteLabel.preferredSize = [
                LABEL_WIDTH + 8,
                ROW_HEIGHT
            ];
            parent.matte = matteRow.add("dropdownlist", undefined, [
                "White",
                "Black",
                "Gray",
                "-",
            ...
            transDitherLabel.preferredSize = [
                LABEL_WIDTH + 8,
                ROW_HEIGHT
            ];
            parent.transparencyDither = tdRow.add("dropdownlist", undefined, ["None", "Diffusion", "Pattern", "Noise"]);
        ...
        onDialogSelect: function(parent) {
            ...
            var ditherType = [Dither.NONE, Dither.DIFFUSION, Dither.PATTERN, Dither.NOISE];
            var WHITE = new RGBColor();
            WHITE.red = 255;
            WHITE.green = 255;
            WHITE.blue = 255;
...
function getFormatOptsBMP() {
    return {
        ...
        dialogParams: function(parent) {
            ...
            var depth = parent.add("group");
            depth.add("statictext", undefined, "Depth:");
            var depthLabels = ["32 bit", "24 bit", "RGB 565 (16 bit)", "ARGB 1555 (16 bit)", "ARGB 4444 (16 bit)"];
        ...
        onDialogSelect: function(parent) {
            ...
            var resolution_enum = [BMPDepthType.THIRTYTWO, BMPDepthType.TWENTYFOUR, BMPDepthType.BMP_R5G6B5, BMPDepthType.BMP_A1R5G5B5, BMPDepthType.BMP_A4R4G4B4];
